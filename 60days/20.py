...
# 五类参数
# 定义函数 f，只有一个参数 a，a 既可能为位置参数，也可能为关键字参数，
# 这取决于调用函数 f 的传参。
...
# 查看参数类型：
for name, val in signature(f1).parameters.items():
    print(name, val.kind)
...
# 查看参数类型：
for name, val in signature(f2).parameters.items():
    print(name, val.kind)
# a KEYWORD_ONLY
# b VAR_KEYWORD
# 看到参数 a 只可能为 KEYWORD_ONLY 关键字参数。因为参数 a 前面有个星号，
# 星号后面的参数 a 只可能是关键字参数，而不可能是位置参数。
...
# 方便的同时，也要求开发者遵守更多的约束规则。如果不了解这些规则，
# 函数调用时，可能会出现各种各样的调用异常。
# 常见的异常有以下六类：
'''
...
# f11(a=1, 20.) # SyntaxError: positional argument follows keyword argument
...
# f12(1, width=20., width=30.)#SyntaxError: keyword argument repeated
...
# 调用方法：
f15(a=1)  # {'a': 1}
f15(a=1, b=2, width=3)  # {'a': 1, 'b': 2, 'width': 3}
# 但是，不能这么调用：
